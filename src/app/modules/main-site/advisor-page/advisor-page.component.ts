...
export class AdvisorPageComponent implements OnInit, AfterViewInit {
  ...
  searchPlz() {
    ...
    this.httpClient.get<ZipSearchResponse>(AdvisorPageComponent.PLZ_SEARCH_URL + this.inputPlz).subscribe(data => {
  ...
  loadHealthInformationForPlz(plz: string) {
    this.httpClient.get<HealthInformationByLocationResponse>(AdvisorPageComponent.HEALTH_INFO_BY_ZIP + plz).subscribe(data => {
